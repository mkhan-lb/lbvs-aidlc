...
Present the onboarding guide in the conversation by default. Produce the starter CLAUDE.md only when explicitly requested; agree its destination and preserve existing instructions. Do not rewrite canonical AIDLC artifacts as an onboarding side effect. The two output formats are:
...
When explicitly requested, generate or update a project-specific CLAUDE.md based on detected conventions. If `CLAUDE.md` already exists, read it first and enhance it — preserve existing project-specific instructions and clearly call out what was added or changed.
...
3. **Respect existing CLAUDE.md** — read existing instructions and propose additions; edit only when requested, preserving existing project policy. Call out what's new vs existing.
...
**Action**: Run read-only reconnaissance and mapping → present Onboarding Guide; offer a starter CLAUDE.md draft only if requested
**Output**: Onboarding Guide printed directly to the conversation; no instruction-file write from the onboarding request alone
