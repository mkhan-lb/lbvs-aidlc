...
When the project already uses `eslint-plugin-react-hooks`, follow its configured rules; configuring new tooling requires separate scope. Upstream recommends `react-hooks/rules-of-hooks` set to error.
...
Illustrative rules (respect existing project configuration):
...
Upstream recommends treating `exhaustive-deps` warnings as errors in CI for new code; do not alter the project’s CI policy without explicit scope.
