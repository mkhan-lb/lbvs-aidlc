...
**Execution scope:** First discover the available `kubectl` and confirm the explicitly selected cluster/context, namespace, and permitted operation. Stop if the required tool or scope is unavailable; do not install tools or switch the session context automatically. Authorized `get`, `describe`, `logs`, and `top` inspection does not authorize `apply`, `create`, rollout undo, scaling, or workload execution. `exec`, port forwarding, server-side dry-runs, secret creation, migrations, and deployment examples require their own applicable authorization; server-side dry-runs still contact the cluster. Never use the illustrative secret values in real deployments or put real secrets into command history.
...
- Optional upstream-only [docker-patterns](https://github.com/affaan-m/ECC/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/docker-patterns/SKILL.md) — Multi-stage Dockerfiles and image security.
- Optional upstream-only [deployment-patterns](https://github.com/affaan-m/ECC/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/deployment-patterns/SKILL.md) — CI/CD pipelines, rollback strategy, health check endpoints.
- Optional upstream-only [security-review](https://github.com/affaan-m/ECC/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/security-review/SKILL.md) — Broader security hardening context.
- [git-workflow](../git-workflow/SKILL.md) — Optional, manually selected Git workflow reference; GitOps controllers are separate integrations, not installed by this skill.
