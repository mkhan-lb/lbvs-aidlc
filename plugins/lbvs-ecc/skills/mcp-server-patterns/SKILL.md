...
For the broader routing decision of when a capability should be a rule, a skill, MCP, or a plain CLI/API workflow, see [bundled upstream capability-surface-selection reference](references/capability-surface-selection.md).
...
**Integration scope:** Discover the actual available documentation tools and installed SDK version before making calls. Context7 is optional, not assumed present; use accessible official documentation instead, and stop API-specific implementation if authoritative signatures cannot be obtained. Server/tool installation, registration in client settings, starting a server, and remote writes require applicable explicit authorization. Read-only resource access never grants tool mutation authority. Do not invent tool names or SDK overloads.
...
Register tools and resources using the API your installed SDK version actually provides. Verify the exact registration overloads and resource-handler arguments against the matching official MCP SDK documentation or an available Context7 integration before implementing them; do not assume interchangeable positional or object overloads.
