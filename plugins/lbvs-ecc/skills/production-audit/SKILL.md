...
### Access and effect boundaries

Discover actual available repository, CI, browser and provider tools and read their current schemas before use; tool availability or credentials do not grant scope. If an evidence source/tool is unavailable, report it as missing and stop that path instead of inventing results. Default to local read-only inspection. Separately agree any network destination, account/environment, credentialed read and data exposure. Do not collect secrets/customer records, run migrations, seed/backfill data, rotate credentials, install scanners, trigger CI/deployment, post comments or change production settings as part of an audit. Tests and build scripts may mutate state or call services: inspect them and select a safe local/test environment before any authorized run.
...
If a deployed URL is explicitly in scope, limit browser or HTTP checks to agreed read-only routes at that URL. A supplied test account is not permission to submit forms or change data: select any credentialed read separately, and require separate authorization and a safe test environment for writes, payment flows, load/security probes or destructive checks.
...
- Bundled reference: [security-review](../security-review/SKILL.md)
- Optional upstream reference (not required by this audit): [deployment-patterns](https://github.com/affaan-m/everything-claude-code/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/deployment-patterns/SKILL.md)
- Optional upstream reference (not required by this audit): [e2e-testing](https://github.com/affaan-m/everything-claude-code/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/e2e-testing/SKILL.md)
- Optional upstream reference (not required by this audit): [tdd-workflow](https://github.com/affaan-m/everything-claude-code/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/tdd-workflow/SKILL.md)
- Optional upstream reference (not required by this audit): [verification-loop](https://github.com/affaan-m/everything-claude-code/blob/8321021c54d670126ce3b2969d5deb880b4b0c2a/skills/verification-loop/SKILL.md)
