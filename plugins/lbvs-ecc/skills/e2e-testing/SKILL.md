...
**AIDLC integration:** Follow the [optional ECC skill library guidance](../../../docs/USAGE.md#12-use-the-optional-ecc-skill-library). Existing project conventions and canonical AIDLC artifacts prevail. Skill use is not authority for installs, commits, remote writes, deployment, or global/session changes. Keep the app’s test framework and CI conventions; install/configuration snippets are illustrative, not automation. Discover available browser/test tools and stop the affected step if unavailable. Read-only inspection does not authorize submissions, trades, or other writes; mutating flows require an explicitly authorized disposable test environment.
