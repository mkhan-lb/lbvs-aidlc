...
Discover the installed Git/hosting tools, exact repository, current branch, local changes, remotes and existing project policy before choosing commands. If a needed capability is missing, stop and report it; do not install tooling or change authentication. The examples below are alternatives for a selected operation, not a run-all checklist. Read-only review never authorizes staging, stash, checkout, merges, rebases, commits, tags, deletion, pushes or deployments. Preserve unrelated user changes; stage only the intended reviewed paths. Never bypass hooks or permission prompts.

History rewrites and destructive examples require specific authorization and a recovery plan; never use them on shared/protected history or someone else's work. `--force-with-lease` is not permission to force-push. Existing branch/release policy overrides the generic strategies below.
...
- Deploy after merge only through the project's authorized release process; a merge is not deployment authorization.
...
- Supports multiple deployments per day when authorized by the project release process.
...
If the user requests a commit-template change and no existing project template applies, propose `.gitmessage` in the repo root:
...
Enable only when separately authorized, repository-locally: `git config --local commit.template .gitmessage`.
...
Inventory candidates first and obtain authorization for explicit branches. The bulk pipeline below is illustrative, not a safe default: it does not establish branch ownership or protect all project branches. Never run blanket deletion on user work. Remote deletion is a separate remote write.
...
Use an existing project generator if available. The `npx` example can download and execute a package; do not run it without separately approved package execution, and never use it as automatic setup.
...
These are optional upstream user-level examples, not defaults to apply. Do not write `~/.gitconfig` or run `--global` commands from this skill. If configuration is explicitly requested, prefer the existing project mechanism or an authorized `--local` setting. Global changes require a separate user request.
...
The following hook snippets are upstream illustrations only. This bundle does not install hooks or assume npm scripts exist. Do not create/replace hooks as part of using this skill; preserve the project's current checks. Hook installation is a separate requested change requiring command and side-effect review. The illustrative regex is not a comprehensive secret scanner.
