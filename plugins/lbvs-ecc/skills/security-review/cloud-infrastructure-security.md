...
**AIDLC integration.** See [the optional ECC skill library](../../../docs/USAGE.md#12-use-the-optional-ecc-skill-library). Existing project conventions and canonical AIDLC artifacts prevail. This supporting checklist is reference guidance, not authority for installs, commits, remote writes, deployment or global/session changes. Review local configuration by default; select any cloud/account read explicitly and discover actual tool/API schemas. Missing capabilities are evidence gaps, not permission to install tooling. IAM, secrets rotation, network policies, logging, backups, CI and deployment mutations require separate authorization and a safe target. Do not run the mutation examples as review checks.

Examples are schematic and incomplete, not drop-in implementations or certification evidence. Match SDK versions, actual resource definitions, account/environment and current provider documentation before separately authorized use; do not infer working APIs from pseudocode. In particular, the CloudWatch excerpt assumes an application-provided client and the WAF shell block is only a comment checklist, not WAF configuration.

This checklist helps review cloud infrastructure, CI/CD pipelines and deployment configurations against security practices; it does not certify compliance with industry standards.
...
Illustrative upstream workflow: do not activate it as supplied. Select and review every action and pin it to an immutable commit before adoption (especially the mutable `trufflehog@main` reference); reconcile the project's actual permissions and OIDC requirements. Enabling or changing CI is a separately authorized operation.
...
{
  "scripts": {
    "ci:install": "npm ci",  // Invoke explicitly; never recurse through the install lifecycle hook
