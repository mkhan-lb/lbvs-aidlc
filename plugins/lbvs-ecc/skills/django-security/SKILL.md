...
`SECURE_BROWSER_XSS_FILTER` was [removed in Django 4.0](https://docs.djangoproject.com/en/4.0/releases/4.0/#securitymiddleware-no-longer-sets-the-x-xss-protection-header).
Do not rely on legacy `X-XSS-Protection`; use escaping and a project-appropriate CSP.
...
// AJAX runs after the form's {% csrf_token %} input is rendered.
// With CSRF_COOKIE_HTTPONLY=True, JavaScript cannot read the cookie.
...
See [Django's token-source rules](https://docs.djangoproject.com/en/5.2/howto/csrf/#acquiring-the-token-if-csrf-use-sessions-or-csrf-cookie-httponly-is-true).
The example requires the rendered token input; a different frontend must deliberately provide its own documented token bootstrap.
