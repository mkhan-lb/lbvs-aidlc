...
First line, verbatim: `**Critic:** aidlc-test-critic`. Then the R-ID coverage table; numbered findings `C1, C2 …`, each with severity (Important | nit), pass number, evidence and the smallest test change that resolves it; `Not examined: <files or R-IDs and why>`. Keep it under one page; a partial critique beats none.
