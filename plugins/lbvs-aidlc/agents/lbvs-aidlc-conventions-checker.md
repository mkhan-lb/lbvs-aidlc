...
2. **Diff vs conventions.** Naming and layout against Conventions and Layout; hand edits under Generated paths; files outside the plan's scope; credential-looking strings (keys, tokens, connection strings, `.env`); large or binary additions. Run `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/aidlc.py" lint-artifacts --root <repo> <change-id>` (from the package root when the target is another repository); each hit is Important at its `path:line`.
