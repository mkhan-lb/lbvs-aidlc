...
| Report a workflow problem | `/lbvs-aidlc-report [bug \| request] [summary]`, which drafts a `process-bug` or `workflow-request` issue for the package repository from `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/aidlc.py" report-context` and files it on **File it** | AskUserQuestion for kind, duplicates and filing; `gh issue list` and `gh issue create` | `github` (issue search and create when `gh` is absent) |
