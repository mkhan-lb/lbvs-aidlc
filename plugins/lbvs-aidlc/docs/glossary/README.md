...
- The `glossary-context.sh` SessionStart hook prints the company's index (`<company>-index.md`: one line per term with aliases, domain and the first sentence of the definition, then the naming traps) on the first turn of every session as `[aidlc-glossary]`. `scripts/build_plugin.py` generates the index from the glossary and `aidlc.py check` fails when they differ, so edit the glossary, never the index. The company is `AIDLC_GLOSSARY`, else `.aidlc/glossary`, else the profile's `Company:` line; `off` silences the block. The full file is read only when the index line is not enough.
- `lbvs-aidlc-intent` and `lbvs-aidlc-design` use glossary terms verbatim in `intent.md` and `spec.md` (EARS lines name the system and its documents the way the glossary does). A word the index lacks goes on the `Glossary terms:` line of `intent.md` and `spec.md` with a one-line definition and is offered as a draft entry from [`template.md`](template.md), written only on confirmation; it is never coined silently.
