...
| `.claude/skills/` and `.claude/agents/` | Project-scoped skill bundles (orchestrator, stage skills, fix, onboard, learn; manual handoff/resume/ideate) and two subagents (`aidlc-verifier`, `aidlc-repo-scout`): workflow instructions, not enforcement or a custom review engine. The four hooks under `.claude/hooks/` (`check-package`, `project-mode`, `protect-tests`, `worktree-create`) are the only deterministic guardrails. |
