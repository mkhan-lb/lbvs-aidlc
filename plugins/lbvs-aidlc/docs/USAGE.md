...
**Next:** a playbook with `Runs ≥ 3` and `Status: Verified` is a promotion candidate: `/lbvs-aidlc-init` step 5 proposes a repository skill `${CLAUDE_PLUGIN_ROOT}/skills/<repo>-<playbook>/SKILL.md` from it, written only on confirmation — the playbook itself is never auto-converted. A replacement procedure sets `Superseded by <path>` rather than deleting the old file.
