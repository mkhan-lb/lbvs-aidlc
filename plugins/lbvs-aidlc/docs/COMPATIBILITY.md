...
- Worktrees: the EnterWorktree tool and `.worktreeinclude` are Claude Code features; where unavailable, `git worktree add ../<repo>-<change-id> -b aidlc/<change-id>` is the documented fallback and the copied local files must be recreated by hand. `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/aidlc.py" mode` is plain Python and works in any host; only its SessionStart injection is Claude-specific.
