...
1. Read `CLAUDE.md`, `REVIEW.md` (the repository's own file, else `${CLAUDE_PLUGIN_ROOT}/REVIEW.md`), the same-change intent/spec/plan that exist, and relevant current code or working-tree changes. A bounded direct request may supply missing upstream context; name the gap instead of inventing files.
