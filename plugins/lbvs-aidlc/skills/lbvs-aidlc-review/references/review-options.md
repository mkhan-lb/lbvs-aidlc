...
The first is an **example ref range**, not an instruction to assume those branches. Replace it with the observed base/head refs or SHAs whose comparison matches the packet. State the effort explicitly for reproducibility: the tiers in `lbvs-aidlc-review` map **standard** → `high` (first pass), **escalated** → `xhigh` (re-review after a fix pass), **maximum** → `max`, and **cloud** → `ultra` (multi-agent cloud review; costs more and needs the cloud feature). `low` and `medium` remain documented but are not used by the loop. Effort changes coverage/confidence, not authorisation.
...
`/review main` is **focus text**, not a positional branch selector. Noninteractive `/review` without a direct PR reference uses a different headless prompt, not this menu: do not claim a menu selection occurred. OMP's existing `reviewer` agent may be used only if the current catalogue exposes it through a real task mechanism and supports passing the complete packet; see [the task-agent route](#oh-my-pi-task-agent-reviewer) and record that agent invocation rather than pretending `/review` ran. AIDLC creates no new dispatcher.
...
### Oh My Pi task-agent reviewer

When the host catalogue exposes OMP's `reviewer` agent through its task mechanism, the wrapper may invoke it directly: put the complete context packet and the report-only boundaries in the task context — the agent starts blank and inherits nothing from this conversation — and record the invocation as a task-agent review, not as `/review`. The task mechanism has **no effort argument**. Record the tier as `standard (native reviewer; effort not configurable)` for the first pass and `escalated (native reviewer; effort not configurable)` for a re-review after a fix pass: the tier names the loop position, not a provider setting. Never write `/code-review high` or `xhigh` as if that command ran. Escalation is a re-run whose brief states `escalated` and adds the prior report, the agreed fixes and the old/new scope; maximum and cloud tiers are unavailable on this route, so offer the engineer's interactive `/review` instead. Preserve the returned P0–P3 priorities, confidence, locations and verdict verbatim. The agent's output is returned to the conversation, not written to `changes/<id>/review.md`; save it through the ordinary save step only when requested.
...
Only after returned results, save `changes/<id>/review.md` if specifically requested; Read the saved file before claiming a verified save. Content boundary: ${CLAUDE_PLUGIN_ROOT}/docs/ARTIFACTS.md#content-boundary — repository-relative paths, no session references, no workflow or machine narrative. Never update spec/plan, code or policy during review. Fixing agreed IDs and executing affected checks belong to a separate authorised implementation pass. A new review receives old IDs, exact changed scope and available fix evidence. Keep unrechecked findings open/not rechecked; distinguish a claimed fix, evidence-backed resolution and engineer dismissal.
