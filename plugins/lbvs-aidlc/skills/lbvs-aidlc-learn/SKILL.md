...
- Ordinary capture writes one learning in the store, or one playbook under `docs/playbooks/` when the engineer chooses that instead. Nothing here edits root instructions, rules, memory, configuration, code or canonical `changes/<change-id>/` artifacts.
