...
<One paragraph: the subsystem or procedure this agent knows, the question it answers, and which AIDLC stage (design, plan, build, verify, review, fix) delegates to it. Name the gap it fills that aidlc-verifier, aidlc-repo-scout, aidlc-design-reviewer, aidlc-threat-modeler, aidlc-test-critic and the bundled Explore/Plan subagents do not.>
...
<!-- Provenance: created by /aidlc-init (lbvs-aidlc agent maker) on <YYYY-MM-DD> from the repository scout report; the shape mirrors .claude/agents/aidlc-repo-scout.md. Keep this file under 3 KB — plugin agents cost startup context. -->
