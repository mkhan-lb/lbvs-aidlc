...
- Preserve unrelated edits; keep code, tests and docs coherent. Run relevant tests and exercise the changed behavior (UI: inspect the real surface against the design), separating observed outcomes from proposed checks. Before declaring a task done, run the repository's pre-commit/lint; none in greenfield mode → mention `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/aidlc.py" conventions --apply`.
