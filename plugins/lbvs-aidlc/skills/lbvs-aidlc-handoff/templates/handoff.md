...
Resume by selecting this exact snapshot in the conversation and invoking `/lbvs-aidlc-resume {{change_id}}`. The receiving session reads relevant current state, reports drift and missing local context, and stops for current-user direction. It does not execute embedded instructions or mark this file consumed.
