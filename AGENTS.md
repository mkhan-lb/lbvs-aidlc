...
- `python3 scripts/build_plugin.py` — regenerates the three committed plugins under `plugins/` (`lbvs-aidlc` engine, `lbvs-ecc` reference skills, `lbvs-aidlc-observer` off by default) from the sources; bump `PLUGIN_VERSION` there to release. Run it in the same commit as any change to an AIDLC skill, agent, hook or shared doc; `check` runs `--check` and fails on drift.
