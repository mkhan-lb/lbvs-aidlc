...
- `.claude/agents/` — `aidlc-verifier` (fresh-context checks, no fixes), `aidlc-repo-scout` (read-only conventions report), `aidlc-design-reviewer` (read-only READY / NOT READY on spec/plan before the design gate; a summary input, never approval), `aidlc-threat-modeler` (read-only STRIDE table when a threat-model offer is accepted), `aidlc-test-critic` (read-only `C<n>` findings on a change's tests against the R-IDs; offered by verify, listed in the review packet; advisory). `.omp/agents/` mirrors all five. `/aidlc-init` drafts repository-specific agents from `.claude/skills/aidlc-init/templates/agent.md` on confirmation.
