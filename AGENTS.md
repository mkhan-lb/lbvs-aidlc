...
- `python3 scripts/aidlc.py report-context` — the redacted session facts `/lbvs-aidlc-report` files in a `process-bug` or `workflow-request` issue on this repository; workflow observations from an adopting repository arrive as those issues, read them with `/lbvs-aidlc-ticket <issue URL>`.
