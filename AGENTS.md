...
- No commits, pushes, new remotes, global settings changes, bypass-permissions flags or deployments without explicit authorisation in the conversation. Only `/lbvs-aidlc-ship` commits, pushes and opens a PR, and only after the engineer picks that option.
