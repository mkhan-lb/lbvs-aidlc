...
- `python3 scripts/aidlc.py install <repo> [--apply]` / `sync <repo> [--apply]` / `update [--from <pkg>] [--apply]` — scaffold a repository with its own state only (`REVIEW.md`, knowledge-store seeds where no store exists, `changes/`, `.aidlc/manifest.json`) and refresh it later (`update` runs inside the adopter and clones the source recorded in the manifest): report first; never overwrites a differing file; never creates or overwrites `AGENTS.md`, `CLAUDE.md`, `.gitignore`, `.claude/settings.json`, `.worktreeinclude` (prints the plugin declaration and merge hints instead); never commits. Skills, agents, hooks and the helper come from the `lbvs-aidlc` plugin, not from files in the repository.
- `python3 scripts/aidlc.py report-context` — the redacted session facts `/lbvs-aidlc-report` files in a `process-bug` or `workflow-request` issue on this repository; workflow observations from an adopting repository arrive as those issues, read them with `/lbvs-aidlc-ticket <issue URL>`. `report-bug` is the automatic path: hook, helper and adapter crashes file themselves (traceback and context only, fingerprinted, once per project, parked under `.aidlc/reports/` without `gh`).
...
- `python3 scripts/build_plugin.py` — regenerates the three committed plugins under `plugins/` (`lbvs-aidlc` engine, `lbvs-ecc` reference skills, `lbvs-aidlc-observer` off by default), the `.omp/agents/` twins, the marketplace and Codex manifests and `docs/glossary/<company>-index.md` from the sources; bump `PLUGIN_VERSION` there to release. Run it in the same commit as any change to an AIDLC skill, agent, hook or shared doc; `check` runs `--check` and fails on drift. Edit `.claude/agents/*.md`, never `.omp/agents/*.md`; edit `docs/glossary/<company>-glossary.md`, never the index.
...
- `changes/<id>/` ships to the adopting repository: repository-relative paths, environment by kind and version, decisions not transcript, nothing about how the workflow, hooks or the workstation behaved — that goes in `FUTURE_WORK.md` here, or in a GitHub issue filed with `/lbvs-aidlc-report` from an adopting repository. Lessons go to the store the repository already names (`CLAUDE.md`, `docs/repo-profile.md`), never a new `docs/solutions/`.
