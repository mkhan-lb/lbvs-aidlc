...
Company AIDLC package: an engineer-led workflow from Anthropic's AI-native SDLC playbook, exported as a repository template. Artifacts live in `changes/<change-id>/` (`intent.md`, `spike.md`, `spec.md`, `plan.md`, `evidence.md`, `review.md`); knowledge records in `docs/adr/`, `docs/incidents/`, `docs/security/`, `docs/references/`, `docs/solutions/`; platform pointers in `docs/platform/`. Contract: docs/WORKFLOW.md. Recipes: docs/USAGE.md. Reference (decisions, configuration, hosts, MCP, stores, helper): docs/REFERENCE.md. Scope: GOALS.md. Deferred work: FUTURE_WORK.md.
...
- `python3 scripts/aidlc.py profile` — prints `profile: missing|fresh|stale (<n> manifest commits since <rev>)` for `docs/repo-profile.md`, parsing only its header lines; refresh with `/aidlc-onboard`.
...
- `.claude/skills/aidlc-*/` — stage skills plus `aidlc-init` (repository setup wizard: doctor → mode/onboarding → platform facts → MCP/CE auth → code graph → agent maker → ignore patterns → summary; idempotent, every write confirmed, never installs or stores secrets), `aidlc-ticket` (Jira/GitHub intake → confirmed ID + seeded intent, read-only), `aidlc-spike` (time-boxed investigation → `spike.md`, never implements), `aidlc-fix` (bug evidence loop), `aidlc-onboard` (brownfield conventions + `docs/repo-profile.md`), `aidlc-learn` (one durable lesson), `aidlc-ship` (clean review pass → conventions check → commit, push `aidlc/<id>`, PR from `templates/pr-body.md`; asks exactly Commit, push and open PR · Commit only · Stop here, or with an Important K-finding Fix findings first (aidlc-build) · Ship anyway: commit, push and open PR · Stop here; never merges). Manual-only utilities: `aidlc-handoff`, `aidlc-resume`, `aidlc-ideate`. 17 `aidlc*` commands.
...
- Repository context (docs/WORKFLOW.md#repository-context) — before scouting or asking, a stage reads what exists: `docs/repo-profile.md` (facts; check `profile` freshness first), the company glossary under `docs/glossary/` named in the profile's Identity (terms verbatim; an absent term is a "new term" offered as an entry, never coined), `docs/playbooks/README.md` (follow a matching playbook; spike/learn offer to save one; build bumps Runs on confirmation), then `platform.md` and `libraries.md`; scout only what is missing or stale.
...
- `.claude/agents/` — `aidlc-verifier` (fresh-context checks, no fixes), `aidlc-repo-scout` (read-only conventions report and profile draft), `aidlc-design-reviewer` (read-only READY / NOT READY on spec/plan before the design gate; a summary input, never approval), `aidlc-threat-modeler` (read-only STRIDE table when a threat-model offer is accepted), `aidlc-test-critic` (read-only `C<n>` findings on a change's tests against the R-IDs; offered by verify, listed in the review packet; advisory), `aidlc-conventions-checker` (Read/Glob/Grep/Bash; runs only the commands the profile or `CLAUDE.md` documents, never `--fix`; `K<n>` findings + commands table to `/aidlc-ship`; advisory, never edits or commits). `.omp/agents/` mirrors all six. `/aidlc-init` drafts repository-specific agents from `.claude/skills/aidlc-init/templates/agent.md` on confirmation.
