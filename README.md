...
- 10 hooks. At session start: the package check (silent when it passes), the project-mode line, a scaffold-version check and the reply style (caveman lite by default; `.aidlc/style` sets `full`, `ultra` or `off`). On every edit: protection of a failing test while you fix its bug, and the content boundary on `changes/**` and lessons. On `gh` and `git push`: a confirmation prompt before a pull request opens or merges or a force-push runs. On `/lbvs-aidlc*`: one bare change ID or nothing. On worktree creation and removal: descriptive names (`aidlc/vs-1234-order-export`) and removal that keeps uncommitted work.
