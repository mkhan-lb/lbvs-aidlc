...
REWRITES = (
    ...
    (re.compile(r"python3 scripts/aidlc\.py mode"), 'python3 "' + ROOT + '/scripts/aidlc.py" mode'),
...
def main():
    ...
    (PLUGIN_ROOT / "README.md").write_text(
        ...
        "Skills are namespaced `/lbvs-aidlc:<skill>` (`/lbvs-aidlc:aidlc <change-id>` starts a change). Agents: "
        "`lbvs-aidlc:aidlc-verifier`, `lbvs-aidlc:aidlc-repo-scout`. Hooks: project-mode line at session start and "
        "reproduction-test protection while `.aidlc/fix/*.json` exists in the project.\n\n"
        "The plugin does not ship a `CLAUDE.md`; run `/lbvs-aidlc:aidlc-onboard` or `/init` in the adopting repository. "
