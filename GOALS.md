...
- Model- and user-invocable stage skills (`lbvs-aidlc-intent` … `lbvs-aidlc-review`, `lbvs-aidlc-fix`, `lbvs-aidlc-learn`), the entry helpers `lbvs-aidlc-ticket` (Jira/GitHub intake, read-only) and `lbvs-aidlc-spike` (time-boxed investigation into `spike.md`), the exit helper `lbvs-aidlc-ship` (commit, push, PR; always asks; never merges), three manual utilities (handoff, resume, ideate) and four scoped subagents (`lbvs-aidlc-verifier`, `lbvs-aidlc-repo-scout`, `lbvs-aidlc-design-reviewer`, `lbvs-aidlc-threat-modeler`, each with an `.omp/agents/` counterpart) — 16 `lbvs-aidlc*` commands.
