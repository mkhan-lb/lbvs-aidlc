...
**Parity follow-ups (from comparing lb-aidlc-workflows `harness/` and ECC's cross-harness model, 2026-09-18), all four implemented the same day on this branch:** (a) generate the `.omp/agents/*.md` twins from `.claude/agents/*.md` in `build_plugin.py` instead of hand-writing them, and drift-check them; (b) make `omp/aidlc-guards.ts` a thin adapter that feeds the same shell hook scripts a Claude-shaped JSON payload, so protect-tests and the PR guard have one body (the artifact guard and style hook already call the shared scripts); (c) a compliance table in `docs/COMPATIBILITY.md` with ECC's four states (native, adapter-backed, instruction-backed, reference-only) and the unsupported surfaces per host, checked by `aidlc.py check`; (d) a native Codex skills surface (`.agents/skills/`) since the skills are already Claude-format.
...
## F12 — Slack for pull-request announcements and deploy commands

**Decided 2026-09-18.** Slack is for two write actions, both behind an explicit yes in the session: posting a shipped pull request to a channel, and later sending a deploy command. Workflow defects and workflow requests are not Slack messages; `/lbvs-aidlc-report` files them as `process-bug` or `workflow-request` issues on this repository, and the channel that wants to see them subscribes to the repository with the GitHub Slack app (`/github subscribe <owner>/<repo> issues comments`), which needs no code or credentials here.

**Route:** `slack@claude-plugins-official`, which is Slack's own `slackapi/slack-mcp-plugin` (MIT; the marketplace pins commit `8044341`). Its `.mcp.json` declares one hosted server, `https://mcp.slack.com/mcp`, HTTP with OAuth in the browser on first use, the same shape as this package's `atlassian` entry; a workspace admin must approve the MCP integration once. It is installed per engineer, not declared in the plugin's `.mcp.json`: its tool schemas would otherwise load into every session of every adopting repository.

**Now:** nothing in the package uses Slack. `docs/PLUGINS.md` lists the plugin as optional.

**Later, in order:** (1) `/lbvs-aidlc-ship` offers **Post the PR link to <channel>** after the pull request opens, when the Slack tools are present in the session, one message with the PR URL; (2) the deploy command, which waits for F4 (the CircleCI route through `app-delivery-kit`), because a deploy sent from a session is a production action and needs the same gate, evidence and audit as ship; (3) `/lbvs-aidlc-ticket` accepting a Slack thread permalink as a read-only source record cited in `intent.md`. Rules that hold for all three: a Slack post never happens without the engineer's answer in that session, Slack messages are sources cited by permalink and never the record, and nothing posts from a hook.
