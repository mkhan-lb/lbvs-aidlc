...
| Review | `aidlc-review`, root `REVIEW.md` | `/code-review` (bundled; also `/review`), `/security-review` where available, fresh-context subagent for adversarial review | `code-review`, `pr-review-toolkit`, `security-guidance`, `github` (PR comments only with explicit authority) |
