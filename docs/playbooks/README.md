...
- **Promotion.** `Runs` at least 3 and `Verified` → `/lbvs-aidlc-init` step 5 proposes a repository skill `.claude/skills/<repo>-<playbook>/SKILL.md`; it is proposed to the engineer, never auto-written, and the playbook stays as the skill's source.
