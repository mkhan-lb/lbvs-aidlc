...
- Generated `plugins/lbvs-aidlc` (0.2.0) after `install --apply` scaffolded the repository: a `write` of `See /tmp/notes …` to `changes/demo/intent.md` was refused with `AIDLC: changes/demo/intent.md would carry session or machine-local references:` and the file was never created; `git commit --allow-empty` was refused by the then-current commit guard (no UI, so the confirm became a block). That guard was replaced the same day by `pr-guard.sh`, which asks only for `gh pr create|merge|…`, non-GET `gh api …/pulls` calls and force-pushes; its shell and extension detectors were smoke-tested on eight commands each (PR create/merge, `gh api -X POST …/pulls`, `--force-with-lease`, `+refspec` → ask/block; plain commit-and-push chain, `gh pr view`, `gh api …/pulls/1` GET → silent), not re-run under `omp -p`.
...
### Automatic bug reports (2026-09-19)

- Faulted throwaway copy of `protect-tests.sh` (a `raise` as the first statement of `main`) run from the generated plugin with a payload whose content was `SECRET-CONTENT-MUST-NOT-LEAK`: the hook exited 0 with empty stdout; eight seconds later `.aidlc/reports/<fp>.json` pointed at issue #5 with label `process-bug`, the traceback, `PreToolUse Write`, the `report-context` block and `fp:<hash>`. The payload content, home directory and temporary paths did not appear in the body or comments. A second run in the same project made no `gh` call (marker present). A run from a second project added one "Seen again" comment to #5 instead of a new issue.
- `report-bug` with `gh` removed from `PATH` parked `.aidlc/reports/<fp>.pending.json` and printed one line; `scaffold-check.sh` then printed `AIDLC: 1 automatic bug report(s) parked`; `report-bug --pending` with `gh` back filed issue #6 and replaced the parked file with the marker.
- Helper crash path: a plugin copy with a `raise` in `print_mode`, run as `aidlc.py --root <project> mode`, printed the traceback, exited 1 and filed issue #7. Reading #7 back showed two defects in the reporter itself: the component was the `--root` value and the marker went to the current directory. After the fix the same run filed #8 as `aidlc.py mode` with the marker in the project. Titles are redacted as well as bodies since #7's title carried a temporary path.
- Oh My Pi adapter: a forced `TypeError` in the `tool_call` handler propagated to the host (omp shows it as an extension error) and the handler's `guarded` wrapper filed it; with the fingerprint already on #9 the run added a comment. The first attempt reported nothing because the plugin copy of the adapter had not been regenerated; `build_plugin.py --check` would have caught it in the package check.
- Probe issues #5 to #9 were closed with a comment naming them as probes.

Limits: crashes in `style-mode.sh` and `glossary-context.sh` (plain `sh`, no Python) are not reported; they exit silently as before. A defect that makes a hook return a wrong decision without raising is invisible to the automatic path and relies on the stage invoking `/lbvs-aidlc-report bug`. Under Claude Code the detached reporter has not been observed in a real session; it was exercised from a shell with `CLAUDECODE` set. The package repository is public: every automatic report exposes the adopting repository's name and remote.
