...
| `.claude/hooks/worktree-remove.sh` | `WorktreeRemove`: runs `python3 scripts/aidlc.py worktree-remove`; removes only paths under `.claude/worktrees/`, never `--force` (uncommitted work is kept and the refusal reported), deletes only fully merged auto-named `worktree-*` branches. |
...
1. **Mode and onboarding** — `python3 scripts/aidlc.py mode`. Brownfield invokes `/lbvs-aidlc-onboard` (the [recipe below](#onboard-a-brownfield-repository)) and keeps its scout report; greenfield runs `conventions` in report mode and offers **Adopt default conventions (`python3 scripts/aidlc.py conventions --apply`)** / **Keep my own tooling** / **Decide later**, pointing at [`docs/platform/README.md`](platform/README.md) when the service needs cloud delivery.
