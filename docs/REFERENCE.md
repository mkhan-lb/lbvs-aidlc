...
| `report-bug` | Reads a traceback on stdin and files a `process-bug` issue on the package repository: fingerprint from component, plugin version, exception type and call chain; marker `.aidlc/reports/<fp>.json` so a project reports a defect once; an open issue with the same `fp:` gets a comment instead; parked as `.aidlc/reports/<fp>.pending.json` when `gh` is not authenticated, filed later with `--pending`. The five Python hooks, the helper's own crash handler and the Oh My Pi adapter call it detached, so a hook's ten-second budget is untouched. |
...
| `.claude/hooks/scaffold-check.sh` | `SessionStart`: when `.aidlc/manifest.json` records a different `plugin_version` from the running plugin, prints one line asking for `aidlc update`; also counts the automatic bug reports parked under `.aidlc/reports/` and prints one line when there are any. |
