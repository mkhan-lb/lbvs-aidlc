...
- Only `aidlc-handoff`, `aidlc-resume` and `aidlc-ideate` carry `disable-model-invocation: true`; every other AIDLC skill is model- and user-invocable with a `when_to_use` line. No skill may set `allowed-tools`, `hooks`, `model`, `agent` or `context`.
