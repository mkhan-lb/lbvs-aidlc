...
- Register hooks in `.claude/settings.json` (this repository) and in `build_plugin.py`'s hooks.json (adopters); files in `.claude/hooks/` do not autoload. Hooks stay deterministic and mechanical: package check, project mode and scaffold freshness at startup/resume; worktree naming; test protection while `.aidlc/fix/*.json` exists; `ask` on every `git commit`/`git push`; content-boundary lint on `changes/**` and lesson writes; one bare ID on `/lbvs-aidlc*`. A hook never approves, only asks or denies.
