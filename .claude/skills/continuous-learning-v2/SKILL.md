...
**AIDLC integration.** See [the optional ECC skill library](../../../docs/USAGE.md#12-use-the-optional-ecc-skill-library). Existing project conventions and canonical AIDLC artifacts prevail. Skill use is not authority for installs, commits, remote writes, deployment, or global/session changes. This package registers no observation hooks and never writes them to a `settings.json`: `hooks/observe.sh` runs only when the engineer enables the separate marketplace plugin `lbvs-aidlc-observer@lbvs-aidlc` (`claude plugin enable lbvs-aidlc-observer@lbvs-aidlc` in Claude Code, `omp plugin enable` in Oh My Pi; shipped `defaultEnabled: false`). `/lbvs-aidlc-init` reports whether it is enabled and prints that command; it runs nothing. Observations (tool inputs and outputs) and instincts live under `${XDG_DATA_HOME:-~/.local/share}/ecc-homunculus/`, outside every repository. `/lbvs-aidlc-learn` remains the confirmed, per-change lesson in `docs/solutions/`; instincts are personal until an engineer promotes one through review. The `/instinct-*`, `/evolve`, `/promote` and `/projects` commands in `commands/` are reference copies, not registered project commands — run `scripts/instinct-cli.py` directly. The manual `settings.json` route described below is not the AIDLC route.
