...
**AIDLC reference boundary:** This is preserved historical upstream ECC maintainer guidance, not an executable AIDLC runbook or authority to release ECC. Repository names, versions, run IDs, scripts, tests, marketplace commands and package names below belong to ECC at the pinned source revision. They are not bundled dependencies or assumed local capabilities. Do not execute these examples against AIDLC or install ECC. Use the selected project's existing release policy; discover actual tools and authorize each tag, push, package execution/publication, announcement and ticket mutation separately. Read-only registry inspection must not silently execute `npx` packages or installation canaries.
