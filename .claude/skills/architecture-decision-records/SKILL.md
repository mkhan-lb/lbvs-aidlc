...
**AIDLC integration:** See [optional skill library](../../../docs/USAGE.md#12-use-the-optional-ecc-skill-library). The store already exists: write `docs/adr/NNNN-short-title.md` from [`docs/adr/template.md`](../../../docs/adr/template.md) and add a row to [`docs/adr/README.md`](../../../docs/adr/README.md) (never renumber or delete — supersede); do not re-initialise the directory. Present the draft and only write after explicit confirmation (AskUserQuestion), then Read-back the written file. Never invent context, alternatives, deciders, dates or approvals — record only what the conversation, the change's `changes/<id>/` artifacts or the repository actually show; mark unknowns as open. Where the text below names the ECC "Planner agent" and "Code reviewer agent", read `lbvs-aidlc-design`/`lbvs-aidlc-plan` (which offer an ADR when a spec introduces a boundary, technology choice or contract) and `lbvs-aidlc-review` (which reports an architectural change without an ADR as a finding, not a blocker); AIDLC does not install those agents. Skill use is not authority for installs, commits, remote writes, deployment or global/session changes.
