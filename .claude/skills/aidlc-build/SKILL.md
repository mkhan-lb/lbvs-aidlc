...
- Preserve unrelated edits; keep code, tests and docs coherent. Run relevant tests and exercise the changed behavior (UI: inspect the real surface against the design or agreed behavior), separating observed outcomes from proposed checks. Before declaring a task done, run the repository's pre-commit/lint; none in greenfield mode → mention `python3 scripts/aidlc.py conventions --apply`.
