...
Contract: docs/WORKFLOW.md plus the bundled [review options](references/review-options.md) and [report template](templates/review.md). Paths are repository-root relative; bundled files are relative to this skill directory.
...
3. Fill the [context packet](references/review-options.md#complete-context-packet): intended behavior and acceptance boundaries, artifact contents, code/callers/tests, actual check results with the revision they exercised, prior finding IDs, concerns, and the applicable `REVIEW.md` policy (severity, exclusions, Bugs/Security/Compliance passes). Compliance includes **architectural change without ADR**: a new/changed architectural boundary, technology choice or contract with no matching `docs/adr/NNNN-*.md` (index `docs/adr/README.md`). A path alone is not delivered content.
4. Keep a scope snapshot (revisions, diff/untracked inventory, time); recheck it before dispatch and after return, naming stale files instead of claiming coverage.

## 2. Select and invoke, or hand off honestly

Default to **Claude Code's bundled local `/code-review` when actually exposed**; an explicitly selected existing OMP session may use its `/review`. Check the real tool/skill/command catalogue and record what is exposed. Use the exact [provider recipes](references/review-options.md#provider-recipes); pass the packet and report-only boundaries through a supported context channel. Claude's non-`ultra` trailing arguments are a **target**, not a brief, and its fork need not inherit this conversation — do not invent flags or assume context transfer.

When the reviewer or a suitable context channel is unavailable, return `prepared — not run`, the provider and its limitation, the complete packet and documented command separately, the user's required menu/transfer step, and a request for native findings and coverage. Never substitute your own review or fabricate a run.

## 3. Preserve and reconcile the result

Wait for an invoked review to return. Copy each native finding body **verbatim** with location, severity/confidence, evidence, verdict and caveats; give findings stable local IDs; put policy mapping, disposition and wrapper observations in separately labelled fields. Record status `prepared — not run`, `running`, `returned`, `partial` or `failed` with scope freshness current/stale/unknown, inspected and skipped scope, missing coverage, errors. Partial or unavailable review is never a clean bill; a provider "correct" verdict is bounded, not approval. An ADR gap is a Compliance finding with its own ID at finding severity, never a blocker; its correction is the `architecture-decision-records` skill in a later pass.
