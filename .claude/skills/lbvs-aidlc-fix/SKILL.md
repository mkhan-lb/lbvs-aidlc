...
While the marker exists the hook (`.claude/hooks/protect-tests.sh`; Oh My Pi `.omp/hooks/pre/aidlc-guards.ts`) denies Edit/Write/MultiEdit/NotebookEdit on listed paths (exact or glob). Probe with a no-op Edit on a protected file: refused → tell the user it is in force; processed → stop, report "protection not in force", do not implement. Believe the test wrong: stop and explain; the user lifts protection by deleting the marker. Never work around the hook (redirection, renaming, a second copy).
