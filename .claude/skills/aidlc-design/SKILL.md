...
1. Read `CLAUDE.md`, the [spec template](templates/spec.md), `changes/<change-id>/intent.md`, any existing `spec.md` and linked source records (authorised access only). If intent is missing, ask for the real task context or suggest `aidlc-intent`; never invent the upstream artifact. Repository context: read per docs/WORKFLOW.md#repository-context before scouting; scout only what is missing or stale.
