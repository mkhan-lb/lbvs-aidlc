...
1. Read `CLAUDE.md`, `REVIEW.md`, the [report template](templates/review.md), and any `changes/<change-id>/intent.md`, `spec.md`, `plan.md`, prior `review.md`, verification record; read each [review options](references/review-options.md) section at the step that links it. Report missing context; never invent approvals, requirements or evidence. Repository context: read per docs/WORKFLOW.md#repository-context before scouting; scout only what is missing or stale.
