...
when_to_use: The `style-mode` hook already applies level lite to chat replies at session start. Load this skill when the engineer asks for another level (`caveman full`, `caveman ultra`), says "normal mode" or "stop caveman", or asks what mode is on.
metadata:
...
The `lbvs-aidlc` plugin applies this skill's **lite** level to chat replies by default, in Claude Code through the `style-mode.sh` SessionStart hook and in Oh My Pi through the guard extension, reading the level from `.aidlc/style` in the repository (`lite`, `full`, `ultra`, `off`; missing means `lite`; the `AIDLC_STYLE` environment variable overrides). Compression applies to chat replies only: code, commits, PR bodies, `changes/<change-id>/` artifacts, lessons, ADRs and every other written record keep normal prose and follow the `unslop` rules. Caveman's own Auto-Clarity list wins over compression, and the stage gates' AskUserQuestion option labels are never compressed. Upstream text below is kept byte-for-byte; provenance in [docs/vendor/caveman/manifest.json](../../../docs/vendor/caveman/manifest.json).
