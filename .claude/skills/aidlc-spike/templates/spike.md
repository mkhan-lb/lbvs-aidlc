...
Record a time-boxed investigation. Replace instructions with what was actually observed; do not invent sources, measurements or decisions. Follow `docs/WORKFLOW.md`. This record informs an intent, spec or ADR — it approves nothing and changes no code.
...
One of: create or refresh `changes/{{change_id}}/intent.md` (`aidlc-intent`), record an architecture decision (`architecture-decision-records` → `docs/adr/`), a further spike with a narrower question, or stop. Name the artifact that would receive this record's conclusions.
