...
4. `git log --oneline <default>..HEAD`: exactly one `test(<change-id>): reproduce …` commit is accepted (docs/WORKFLOW.md#reproduction-commit); any other commit ahead is listed and needs the engineer's confirmation.
